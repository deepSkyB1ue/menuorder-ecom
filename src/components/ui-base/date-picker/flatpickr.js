...
function Flatpickr(element, config) {
    ...
    function createDay(className, date, dayNumber) {
      ...
      ) { self.minRangeDate = date; } else if (
        self.selectedDates[0]
      ...
      (self.config.maxDate && compareDates(date, self.config.maxDate) > 0)
        ...
        for (let i = 0, d; i < array.length; i++) {
            ...
            if (d instanceof Function && d(dateToCheck)) // disabled by function
                { return bool; } else if ((d instanceof Date || (typeof d === 'string')) &&
    ...
    function parseConfig() {
        ...
        const userConfig = Object.assign({}, self.instanceConfig, self.element.dataset || {});
        ...
        if (date.toFixed || timestamp.test(date)) // timestamp
            { date = new Date(date); } else if (typeof date === 'string') {
                ...
                if (date === 'today') {
                    date = new Date();
                    timeless = true;
                } else if (self.config.parseDate) { date = self.config.parseDate(date); } else if (timeRegex.test(date)) { // time picker
    ...
    function triggerEvent(event, data) {
        ...
        : [self.config[`on${event}`]];
            ...
            for (let i = 0; i < hooks.length; i++) { hooks[i](self.selectedDates, self.input.value, self, data); }
        }
